...
4a. THE **Issue_Dashboard_Page** SHALL 依議題的 `start_date`（議題建立日）判斷所屬月份，僅統計
   `start_date` 落在目前所選月份內的議題；WHEN 使用者切換月份，THE **Issue_Dashboard_Page** SHALL
   重新計算「依專案分類」統計表；WHEN 所選月份無任何符合的議題，THE **Issue_Dashboard_Page** SHALL
   顯示「所選月份無議題資料」，不留白。
   （**設計變更紀錄**：原設計此表「不受月份選擇影響、恆為全量統計」，理由是與月度 KPI 的月結性質
   做區隔；使用者回饋此表應與月度 KPI 一樣依所選月份呈現，故改為依 `start_date` 月份篩選。）
4b. THE **Issue_Dashboard_Page** SHALL 提供「依專案分類」統計表的客訴／測試／其他／總計欄位排序功能：
   WHEN 使用者點擊欄位標題，THE **Issue_Dashboard_Page** SHALL 依該欄位數值排序表格列；WHEN 使用者
   再次點擊同一欄位標題，THE **Issue_Dashboard_Page** SHALL 反轉排序方向；WHEN 使用者點擊不同欄位
   標題，THE **Issue_Dashboard_Page** SHALL 預設以該欄位數值由大到小排序；THE **Issue_Dashboard_Page**
   SHALL 在目前排序中的欄位標題顯示排序方向指示（▲／▼）；專案欄位不提供排序。
5. THE **Issue_Dashboard_Page** SHALL 在月度 KPI 區塊顯示說明文字，明確告知：KPI 卡片為月結數字
   （當月進行中尚未結算），而每日趨勢與依專案分類統計則依此處所選月份即時呈現，「議題資料」分頁的
   議題明細不受月份篩選影響（顯示全部議題）——避免使用者誤以為「當月無資料」是月度 KPI 尚未結算
   所致，實際上每日趨勢與依專案分類統計已依所選月份即時呈現。
6. THE **Issue_Dashboard_Page** SHALL 確保此說明文字僅出現在月度 KPI 區塊，不得重複出現在議題明細
   的專案／狀態篩選控制項附近，避免使用者誤解為「篩選功能未生效」（專案／狀態篩選實際上會影響下方
   議題明細清單，僅「月份」不影響議題明細清單）。
...
1. THE **Issue_Dashboard_Page** SHALL 以簡易折線圖（純 SVG 或 `<canvas>`，不引入圖表框架）呈現每日
   趨勢資料（客訴／測試／其他／總計）。
1a. THE **Issue_Dashboard_Page** SHALL 僅呈現所選月份內的每日趨勢資料點；WHEN 使用者切換月份，
   THE **Issue_Dashboard_Page** SHALL 重新繪製趨勢圖；WHEN 所選月份無任何每日趨勢資料，THE
   **Issue_Dashboard_Page** SHALL 顯示「所選月份無每日趨勢資料」，不留白。
   （**設計變更紀錄**：原設計此圖「不受月份選擇影響、恆為全量資料」；使用者回饋每日趨勢應依所選
   月份呈現，故改為依月份篩選 `daily_kpi` 資料列。）
...
3. WHEN 使用者將滑鼠移至（或觸控點選）圖上某一資料點，THE **Issue_Dashboard_Page** SHALL 顯示該日期
   的詳細數值（tooltip 或等效呈現方式）；瀏覽器原生 `<title>` 提示框即足以滿足此需求，不需另外實作
   自訂 tooltip 元件。
4. THE **Issue_Dashboard_Page** SHALL 顯示縱軸數值刻度（0、中間值、最大值三條水平格線與對應數字）。
5. THE **Issue_Dashboard_Page** SHALL 為每一個資料點顯示橫軸日期標籤（不省略、不限制數量），並將
   標籤以 -45 度旋轉呈現（`text-anchor: end`），避免密集資料點造成標籤互相重疊。
   （**設計變更紀錄**：原設計「資料點數量超過 6 個時等距挑選標籤（含首尾）」；使用者回饋希望每個
   資料點都能看到日期，故改為顯示全部標籤並以旋轉方式避免重疊，取代原本的等距抽樣做法。）
...
5. WHEN 篩選後無符合條件的議題，THE **Issue_Dashboard_Page** SHALL 顯示「目前無符合條件的議題」，
   不留白。
6. THE **Issue_Dashboard_Page** SHALL 排除 `tracker` 欄位值為「測試」的議題，不將其列入本頁面任何
   品質相關呈現——包括議題明細清單、「依專案分類」統計表（需求 2.4／2.4a）——因 `tracker=測試` 為
   測試性質議題（例如測試環境驗證、測試資料回填），非真實缺陷或客訴，不應計入品質統計。此排除規則
   在資料載入階段即整批套用，而非以篩選控制項呈現（不同於專案／狀態篩選，使用者無法將其重新納入
   顯示）。
...
區塊。改為兩個分頁籤呈現，篩選控制項各自歸屬到其實際影響的分頁籤內，從畫面結構上就消除此歧義。

（**設計變更紀錄**：原本「依專案分類」歸類在「議題資料」分頁籤（與議題明細並列，因兩者當初都被
視為「即時全量資料」）；後續 4a 需求變更為依月份篩選後，「依專案分類」與月份選擇的關聯性更接近
月度 KPI／每日趨勢，故改為歸類到「統計摘要」分頁籤，與月份選擇控制項放在一起；「議題資料」分頁籤
現僅保留不受月份篩選影響的議題明細。）

#### 驗收標準

1. THE **Issue_Dashboard_Page** SHALL 以兩個分頁籤呈現內容：「統計摘要」（月度 KPI ＋每日趨勢＋依
   專案分類統計）與「議題資料」（僅議題明細）。
