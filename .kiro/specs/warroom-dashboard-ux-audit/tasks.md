...
- [x] **摘要卡缺少「目前套用了哪些篩選」的說明** —— 摘要卡上方加一行 `.filter-summary`
      文字說明。加了說明文字之後，使用者實際試用發現底下講的「不受範圍／日期區間影響」
      這件事本身就不合理：選「範圍＝本週到期」自然預期看到「這週的統計」，卻仍是全部任務
      總數。與其解釋一個違反直覺的行為，不如改行為本身——摘要卡改成套用「專案」「任務
      類型」「範圍」「預計完成日期」**全部**篩選條件，統計 Actor#call 裡跟 `display_data`
      同一份 `filtered`（原本另外用 `scoped_tasks` 只套用專案／類型）。文字改為「以上統計
      反映目前的篩選結果（專案、任務類型、範圍、預計完成日期）；『逾期』計入目前仍逾期，
      或已完成但當初遲交的任務。」同步更新 `warroom-data-api-real-source/requirements.md`
      需求 7，並在 3 個原本假設「摘要不受範圍影響」的 rspec 案例加上明確 `scope: "all"`
      （這些案例驗證的是狀態正規化／逾期定義寬窄，不是範圍篩選本身，避免被新行為誤傷）。
